...
### decrypt_verify #########################################
  ...
  sub decrypt_verify { my ($this,$passphrase,$text) = @_;
    ...
    $verify->{'text'}     = $output;
		...
		for (my $i = 0; $i < $#text; $i++) {
			if ($text[$i] =~ 
				/gpg: encrypted with (\w+).+, ID (\w+), created ([0-9\-]+)/) {
				# ...
				if ($2) {
					push(@{$verify->{'keys'}}, $2);
				}
			}
			if ($text[$i] =~ /gpg: decryption failed: (.+)/) {
				$verify->{'ok'} = 0;
				my $error_msg = sprintf("decryption failed: %s", $1);
...
=head1 INSTALLATION
 ...
 % make install
  ...
  - if you want secure memory, do not forget :
    % chown root /usr/bin/gpg ; chmod 4755 /usr/local/bin/gpg
...
=item I<new %params>
 ...
 Parameters are :
 - gnupg_path (most of time, 'gpg' stand inside /usr/bin)
